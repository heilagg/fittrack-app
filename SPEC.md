...
**Срезы объёма не перемножаются.** На объём мышцы действуют фаза (§11.2) и
утомление (§8.2), и произведение давало бы двойное наказание за одно и то же:

```
volumeFactor = fatigueFactor < 1.0
    ? min(phaseFactor, fatigueFactor)   // из двух срезов берётся сильнейший
    : phaseFactor                        // надбавку фазы получает только свежая мышца
```

Ветвление, а не просто `min`: наивный `min(phaseFactor, fatigueFactor)` при
...
- 25b. Композиция объёма на пяти случаях §10: свежая + ранняя лютеиновая → 1.15;
  свежая + менструальная → 0.75; утомлённая + менструальная → 0.70; утомлённая +
  ранняя лютеиновая → 0.70; частично утомлённая + менструальная → 0.75
