...
**Промах по `kid` — один перезапрос JWKS, не чаще раза в 60 секунд.** Без него
ротация ключей давала бы окно сплошных 401 у всех разом, посреди тренировок, —
а §20.5 продаёт асимметрию именно тем, что ротация проходит без деплоя. Троттл
нужен, потому что перезапрос инициирует неаутентифицированный запрос: без
ограничения это внешний рычаг на исходящий трафик сервера. Второй промах подряд
— 401, без повторного похода.
