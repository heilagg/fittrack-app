...
**Целевой вектор.** Задаётся типом дня и акцентом; доли — от эффективного объёма
сессии. Пример «низ тела, акцент ягодицы»:
...
Векторы для остальных типов дня (full body, верх, пуш, пул) и правило, как акцент
меняет вектор, не заданы — открытый вопрос §19.2, п.12.

**Объём сессии.** Вектор задаёт форму, §7.4 — масштаб:

```
ведущая мышца дня = акцентная, иначе мышца с наибольшей долей вектора
S_эфф   = норма-старт(ведущая мышца, §7.4) / Σ доля_сессии[ведущая] по сессиям недели
...
набирает 11 подходов и 26 минут из шестидесяти (прогон). Дотягивать объём до
...
`skill_level` — не ослабляется никогда. Ослабляется требование трёх паттернов: до
того числа, которое доступно, с `ReasonCode` наружу. Время не превышается.
Сценарий 30 (§18) — именно этот случай: при травме колена на низ остаются только
шарнирные и изоляция, и тренировка собирается из них.
...
score = − w1  · Σ по мышцам      |факт[m] − цель[m]|
        ...
        −       Σ по упражнениям min(w8 · block_mismatch + w9 · complexity_at_low_readiness,
                                     max(w8, w9))
        − w10 · Σ по упражнениям (упражнение «на поддержании» ? 1 : 0)
...
| w10 | 1.0 | упражнение «на поддержании» уступает альтернативе при разнице до 1.0 |
...
3. Если объём не принимает даже минимум (глубокий срез), снимается упражнение,
   ...
   минимум паттернов.
4. Сюда же ложатся добавленные подходы §9.5, п.2 (`extraSetsAdded`; где он
   хранится — §19.2, п.7).
...
Обычная тренировка — 4–7 упражнений; бюджет времени опускает до трёх (сценарий 29:
при `session_minutes = 20` собирается 3 упражнения на 19 минут).
...
дня** (акцентной, иначе с наибольшей долей вектора). Остальные мышцы получают
объём по форме вектора §7.3, и он у них ниже нижней границы: в прогоне средний
уровень с акцентом на ягодицы набирает за неделю 16.6 подхода на `glute_max` и 8.0
на квадрицепс. Это не недосмотр: доли вектора отличаются в 6–7 раз, границы
таблицы — в полтора, и выполнить их одновременно нельзя. Приоритет у формы дня,
...
7. **Где живёт счётчик добавленных подходов** (`extraSetsAdded`, §9.5, шаг 2)?
   ...
   планировщик и передавать текущее значение на вход §9.5. Место в порядке §7.3
   определено (между раскладкой подходов и ±1 готовности); открытым остаётся
   только хранение.
...
11. **Словарь инвентаря упражнения** (§6.2, §7.5). `equipment` — список строк
    ...
    которого нет.
12. **Целевые векторы для остальных типов дня** (§7.3). Заданы только два примера
    «низа» — с акцентом и без. Нет векторов для full body, верха, пуша и пула и
    нет правила, как акцент меняет вектор: поднимает долю акцентной мышцы до
    фиксированной величины или масштабирует остальные. От этого зависит объём
    сессии целиком (§7.3, `S_эфф` считается от доли ведущей мышцы), поэтому
    придумать векторы в тестах нельзя — они станут спекой. Решать вместе с
    разметкой библиотеки (этап 2): и то и другое — контент, который проверяет
    специалист (п.2).
