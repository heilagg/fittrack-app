...
| Изменение уверенности в фазе | Фазовые поправки перемасштабируются (непрерывно, §11.3) |
...
|---|---|---|
| Объём | Недельный, ±% при генерации сетки (§7.4) | Сессия, ±1 подход при выходе за 0.9 / 1.05 |
...
Надбавки от остаточного утомления (§8.2) и флага боли (§8.4) под это правило
**не подпадают** и добавляются сверх. Они измеряют другое — состояние конкретной
мышцы и сустава, а не общее самочувствие дня, — и служат безопасности, а не
подстройке нагрузки. Ограничивать их ради красоты формулы нельзя.
