...
`recencyFactor` сравнивает его ФАКТИЧЕСКУЮ длину с той, что предсказывалась
...
confidenceAtClose(цикл i) =
    ...
    × recencyFactor(фактическая длина i, expectedLength по истории до i)
```

Иначе правило не срабатывает никогда. Если на закрытии считать
`recencyFactor = 1` (свежая менструация по определению не просрочена), то с
третьего измеренного цикла `dataFactor` упирается в 1.0, `regularityFactor` не
...
Направление у `recencyFactor` одно: он измеряет ПЕРЕБОР над прогнозом. Цикл,
пришедший заметно РАНЬШЕ ожидаемого, уверенность на закрытии не снижает —
правило нацелено на циклы, растягивающиеся дальше предсказанного, и на тех, у
кого прогноз стабильно не сбывается в эту сторону. Разброс в обе стороны и так
ловится `regularityFactor` через σ.
