...
Когда опорной даты нет вовсе (§11.3), `cycleConfidence` не определён, но режим
остаётся `phases`, поэтому вторая ветка формулы неприменима: в этом состоянии
`checkinScale = 1.6`, как в режиме без фаз. О цикле не известно ничего, и весь
вес переходит на чек-ин. Без этой оговорки готовность в таком состоянии не
вычислялась бы вовсе.
...
|---|---|---|
| Объём | Недельный, ±% при генерации сетки (§7.2) | Сессия, ±1 подход при выходе за 0.9 / 1.05 |
| RIR | Целевой RIR блока | +1 при `readiness < 0.85` |
| Вес | — | `baseline_kg × readiness` (§9.6) |
| Упражнения | Мягкие штрафы (§11.2) | При низкой готовности — меньше технически сложных |

Надбавки к RIR из этих двух источников **не складываются**: вместе они дают не
более +1. Иначе поздняя лютеиновая фаза (RIR +1) вместе с просевшей из-за неё же
готовностью давала бы +2 и превращала рабочий подход в разминочный. Причина
двойного счёта в том, что источник у них общий: фаза роняет готовность, и та же
фаза отдельно поднимает RIR.
...
(§11.5) в режиме `no_phases` — и утомление (§8.2). Источник планового среза один
из двух, никогда оба сразу, но правило композиции с утомлением у них общее:
...
plannedFactor = phase_mode == phases
    ? phaseFactor[P]                                     // §11.2
    : (текущая неделя разгрузочная ? 0.85 : 1.0)          // §11.5, накопление/разгрузка
```
...
утомлённая → 0.70; разгрузочная неделя + частично утомлённая (0.9) → 0.85.
...
сетки (§7.2), затем ±1 подход на конкретной сессии. Без правила «сильнейший»
...
**Утомление и боль готовностью не управляются.** Ограничения §8 (остаточное
утомление) и §8.4 (флаги боли) — жёсткие фильтры, применяемые к результату
после того, как готовность посчитана. Поэтому `push` не может их снять: он
двигает число, а они отсекают варианты.
...
regularityFactor:  измерено ≥ 2 длин:  σ ≤ 2 дня        → 1.0
                   ...
                   ПОТОЛОК: если из окна исключён выброс → не выше 0.7
...
регулярным. Поэтому полоса расширяется до медианы ± 2 дня: тот же порог
«σ ≤ 2 дня → регулярно», а не новая константа. На примере с квартилями выше
...
**Окно с исключённым выбросом не может быть названо максимально регулярным.**
σ считается по отфильтрованному окну (см. «Область подсчёта»), и для ПРОГНОЗА
это правильно: выброс не должен портить среднее. Но у σ после фильтра есть
свойство, которое легко упустить — выброшенное значение делает остаток ТЕСНЕЕ
обычного, поэтому σ выходит не просто низкой, а близкой к нулю. У истории
28, 28, 28, 28, 28, 40 после отбрасывания 40 остаются пять одинаковых циклов и
σ = 0.00: арифметический максимум регулярности, неотличимый от той, у кого цикл
не сдвигался ни на день. Через `cycleConfidence` это идёт в силу фазовой
поправки и в право интерфейса назвать фазу — то есть приложение говорило бы
уверенно ровно после того, как промахнулось на двенадцать дней.

Поэтому `regularityFactor` ограничен сверху значением 0.7, если из окна исключён
хотя бы один выброс. Это принцип, а не штраф: окно, в котором был выброс, не
бывает максимально регулярным. Своего числа правило не вводит — 0.7 это уже
существующая средняя ступень той же таблицы.

Почему не считать σ по неотфильтрованному окну, очевидная альтернатива: она
перегибает. На том же примере 27, 28, 28, 29, 45 неотфильтрованная σ = 6.83 →
0.4, тот же нижний разряд, что и у цикла, скачущего 45 / 20 / 44 / 21 / 46 / 19.
Четыре ровных цикла и один сбой — не то же самое.
...
- `push` не снимает ограничения безопасности: остаточное утомление (§8) и флаги
  ...
  считается раньше, жёсткие фильтры применяются позже.
