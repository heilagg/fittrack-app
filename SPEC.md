...
акцент не выбран, день собирается по вектору без акцента, и норма акцентной мышцы
(§7.4) не действует. Дефолтные акценты §1.2 — то, что интерфейс предлагает первым,
а не значение, которое сетка ставит сама. Выбор или снятие акцента — плановое
решение (§7.3): меняются вектор дня и знаменатель `S_эфф` недели.
