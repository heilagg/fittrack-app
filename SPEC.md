...
│   ├── FitAPI/                  # DTO границы /v1 в обе стороны (§20.2, §20.3)
...
**`FitAPI` — форма границы `/v1`, вынесенная из сервера в общий пакет.** Ни один
тип `FitCore` не сериализуется сам: там нет ни одного `Codable`, и это не
случайность. Синтезированная форма enum с ассоциированными значениями привязана
к именам случаев в Swift, поэтому переименование `ReasonCode` молча поменяло бы
проволочный формат у обоих развёрнутых клиентов — а §20.8 уже зафиксировала, что
границу такого рода «нельзя поменять никогда». Поэтому DTO объявляются явно, с
ручными `CodingKeys`, и живут рядом с `FitCore`, а не внутри `Server/`: иначе
iOS-клиент напишет второе соответствие ровно там, где §20.2 обещает единственное.
...
create table week_plans (
  ...
  -- Последний ПОКАЗАННЫЙ пользователю план недели (§20.6). Нужен ровно одному
  -- потребителю: Planner.rebuildNotice сравнивает два ВЫХОДА планировщика и без
  -- предыдущего не может решить, печатать ли «План обновлён» (§7.1). Хранить
  -- его больше негде: состав незапущенного дня в схеме не живёт — у будущих
  -- дней нет строки workouts, а workout_exercises висит на ней. Формат —
  -- {"v": 1, "days": {"<day_id>": {"kind": ..., "cause": ...,
  -- "composition": {"<slug>": target_sets}}}}: ровно то, что читает
  -- Planner.isPlanChange, и ничего сверх. Номер версии обязателен с первой
  -- записи по той же причине, что у input_digest, и по более сильной — этот
  -- снимок читают кодом, а не глазами. Пишут его только пути, отдающие неделю
  -- целиком (§20.6); /preview не пишет. Для отладки он не нужен, этим занят
  -- input_digest. null — неделя сгенерирована, но ни разу не показана: тогда
  -- пересборке не с чем сравнивать и строка не печатается.
  last_shown_plan jsonb,
...
create table workouts (
  ...
  -- Баннер §9.8 («Подбираем ваши веса») и аналитика. Расчётным фильтром НЕ
  -- является: калибровка по §9.8 — свойство упражнения, а не тренировки, и
  -- подходы из утомления и из неделя[m] исключает
  -- workout_exercises.is_calibration (§20.6).
...
create table workout_exercises (
  ...
  -- Было ли ЭТО упражнение в калибровке на момент старта — снимок
  -- exercise_states.in_calibration (§9.8). По нему, а не по
  -- workouts.is_calibration, калибровочные подходы исключаются из модели
  -- утомления §8.1 и из неделя[m] §7.3: калибровка кончается по упражнению, и
  -- смешанная тренировка — одно новое упражнение плюс три освоенных — флагом на
  -- тренировке не описывается ни одним его значением. Снимок, а не join к
  -- exercise_states на чтении: к тому времени, когда подходы попадут в свёртку,
  -- состояние уйдёт вперёд, и ответ задним числом стал бы другим.
  is_calibration boolean not null default false,
...
create table exercise_states (
  ...
  -- Границы на момент ПОСЛЕДНЕЙ сессии упражнения, а не текущие: пишутся только
  -- вместе с сессией, из того же вызова Planner.repRange, что дал
  -- workout_exercises.target_rep_* и дерево §20.9 (§9.1, §20.6). Между сессиями
  -- не пересчитываются: состояние после сбросов §9.7 накладывает сборка, и
  -- снаружи его взять неоткуда.
...
**Роль, которой ходит сервер, — часть этой защиты, а не деталь эксплуатации.**
Политики выше проверяют `auth.uid()`, а он берётся из
`request.jwt.claims`, — но и то и другое действует лишь для роли, на которую RLS
вообще распространяется. Роль `postgres` в Supabase имеет `rolbypassrls`, то
есть обходит все политики этого раздела целиком и независимо от
`force row level security`. Поэтому веб-сервер логинится отдельной ролью
`fitserver` без собственных прав, которая умеет только стать `authenticated`
(§20.4, миграция `0008`). Роль живёт на уровне кластера, а не базы, поэтому
миграция обязана заводить её идемпотентно: повторное применение к кластеру, где
роль уже есть, иначе падало бы на `role already exists`. Локальный цикл этого не
показывает — `supabase db reset` кастомные роли удаляет (замерено), и там роль
каждый раз создаётся заново вместе с базой, — так что ветку повторного
применения проверяет отдельный прогон файла, а не `db reset`.
...
Он сеет данные как `postgres` и опирается при этом на тот самый `rolbypassrls`
— то есть проверяет политики, но ничего не говорит о пути сервера. Путь сервера
проверяют 20k и 20a (§20.15).
...
| Пользователь вручную поменял тип или акцент дня | Пересобираются все оставшиеся дни: смена акцента меняет вектор дня, а с ним знаменатель `S_эфф` всей недели (§7.2) |
...
«Все оставшиеся дни» в строке про правку дня — не описка и не расширение
правила. Раньше здесь стояло «только этот день», и это расходилось с §7.2,
который про тот же случай говорит: «меняются вектор дня и знаменатель `S_эфф`
недели». Верно второе. Масштаб сессии считается от долей ВСЕХ силовых дней
недели (§7.3), поэтому снятый или поставленный акцент двигает числа и в
остальных днях; «только этот день» описывало намерение пользователя, а не объём
пересчёта. Практической разницы в вызове нет — `planRemainingDays` пересобирает
оставшиеся дни целиком в любом случае, — но текст обязан называть то, что
происходит.
...
(§13.4), старт тренировки (§20.3, `Planner.prescribe`), — переписывается и
`weight_readiness`.
...
5. Если флаг боли по разным упражнениям с одним `joint_stress`-суставом
   ...
   упражнении, который уже покрыт п.4.

   **Событию приписывается не один сустав, а множество** (§19.2 п.8): все
   суставы упражнения с максимальной степенью нагрузки в его `joint_stress`.
   Счёт п.5 идёт по пересечению множеств — сустав набирает три упражнения, если
   входит в множество каждого из них. Выбирать один сустав из равных было
   нечем: канонический пример §6.2 (`hip_thrust_barbell`) даёт `lower_back` и
   `hip` одинаковой степени `medium`, и любой тай-брейк между ними молча
   обнулял бы счёт второго. Множество пишется в `sets.pain_joints` в момент
   события и позже не пересчитывается: правка разметки `joint_stress` не должна
   задним числом создавать или отменять рекомендацию показаться специалисту.
...
**Упражнение, которого в сессии нет, эти колонки не обновляет.** Состояние
«после §9.7» для него взять неоткуда: сбросы накладывает сборка, и наружу её
результат приезжает только предписанием. Поэтому обе колонки — снимок на момент
ПОСЛЕДНЕЙ сессии упражнения, а не текущее значение; `not null` при этом
выполняется с первой записи, потому что строка `exercise_states` и заводится
вместе с первой сессией. Пересчитывать их между сессиями по сырому состоянию
нельзя — это ровно те `8–15` против `8–12`, о которых абзац выше, только теперь
в колонке, которую никто не сверяет.
...
**Не реализовано (1 из 2).** Суженный диапазон из `increaseWeightWithRepReset`
...
**Не реализовано (2 из 2).** Таблицы стартового веса живут вне ядра прогрессии —
...
- разрешено до 3 повышений внутри одного упражнения за сессию. Счётчик
  передаётся в `Progression.nextSet` параметром (`calibrationIncreasesUsed`):
  сама прогрессия между сессиями им по-прежнему не владеет и ничего не хранит,
  но правило живёт рядом с остальным §9, а не у вызывающего. Исчерпав лимит,
  ветка «легко, повторов не меньше `rep_max`» перестаёт давать ×1.15 и
  оставляет вес без изменения. Раньше ограничение было объявлено обязанностью
  экрана тренировки — на вебе этим экраном оказалось дерево решений §20.9,
  которое строит сервер, и правило неизбежно разошлось бы с iOS, где тот же
  экран вызывает `nextSet` напрямую (§20.9, §20.15 тест 20c)
- результаты калибровочных подходов **не идут** в модель утомления и в
  недельный объём. **Калибровка — свойство упражнения, а не тренировки:**
  кончается она по упражнению (следующий пункт), поэтому и фильтр стоит по
  `workout_exercises.is_calibration` (§3.1) — снимку
  `exercise_states.in_calibration` на момент старта. `workouts.is_calibration`
  остаётся баннеру и аналитике: в смешанной тренировке, где одно упражнение
  новое, а три освоены, ни одно его значение не даёт верного ответа — `true`
  выбросил бы из учёта три честных упражнения, `false` засчитал бы
...
Они вернутся вместе с полным закрытием §9.5, «Не реализовано (1 из 2)».
...
- планировщик использует классическую недельную периодизацию: три недели
  ...
  утомление — открыто (§19.2, п.4). **Пока оно открыто, признак всегда `false`:
  сервер пишет `week_plans.is_deload = false` при каждой генерации, и
  разгрузочная неделя не наступает ни у кого.** Это записано прямо, потому что
  иначе §11.5 читается как описание работающего механизма, которого нет.
- признак хранится в `week_plans.is_deload` (§3.1) и фиксируется в момент
  генерации недели: пересборка (§7.1) читает записанное значение и не считает
  его заново, иначе пересборка с неизменившимся входом меняла бы план от одного
  течения времени.
- признак действует **только в этом режиме**: `plannedVolumeFactor` читает его в
  ветке «без фаз», а в фазовом режиме объём задаёт периодизация §11.2. Отсюда
  обязанность слоя соответствия (§20.6): пользователь без строки `cycle_profiles`
  или с `cycle_tracking = 'off'` обязан попадать именно в режим без фаз, иначе
  разгрузка у него не сработает вовсе и ни один тест этого не покажет.
...
**Где эта логика живёт — в `FitCore`, а не на сервере.** Поля под неё лежали в
срезе планировщика (`alternatives`, `family_load_ratio`, §7.5) с самого начала,
но не читались ни одной функцией. Довод за ядро решающий и не архитектурный:
замена упражнения и флаг боли (§8.4, п. 2) случаются посреди тренировки, а §4.3
требует, чтобы на iOS тренировка шла без сети. Логика, живущая в `Server/`, в
подвале недоступна, и iOS пришлось бы написать её второй раз — ровно то, что
запрещает §20.1.

Разделение труда: `alternatives` из разметки контента — это пул кандидатов, а не
ответ. Ранжирование зависит от пользователя (инвентарь §6.6, ограничения §14.4,
флаги боли §8.4, какой сустав беречь) и потому считается на вызове, поверх тех же
жёстких ограничений §7.3, что и обычный подбор.

**Причина пригодности — код, а не строка.** «Тот же акцент на ягодицы, без
нагрузки на колено» — это два машинных факта: совпала ведущая мышца и снизилась
степень нагрузки на сустав, по которому у пользователя ограничение или недавний
флаг боли. `FitCore` отдаёт именно их, отдельным случаем `ReasonCode`, а не
строкой — по общему правилу §14.6. Готовую формулировку к этому коду добавляет
уже ответ `/v1` (§20.3, «Причины на проводе»), как и всем прочим доменным
причинам.
...
диалога флага боли. Когда событию боли приписано несколько суставов (§8.4 п.5),
диалог предлагает их все и ограничение ставится на выбранный пользовательницей,
а не на первый из множества: `user_restrictions.joint` — это одна строка на
сустав, и приложение не вправе решать за неё, какой из равных по нагрузке
суставов болит.
...
4. **Разгрузочная неделя в режиме без фаз** — каждые 4 недели или по накопленному
   ...
   и сам его не считает (§11.5). **Место хранения закрыто отдельно от правила:**
   `week_plans.is_deload` (§3.1), значение фиксируется при генерации недели. Пока
   правило не выбрано, сервер пишет туда `false`, то есть разгрузочная неделя не
   наступает ни у кого — это записано в §11.5 явно, чтобы раздел не читался как
...
8. **Какой сустав приписывать флагу боли** (§8.4, п.5) — **закрыт: событию
   приписывается не один сустав, а множество** (вариант «б»). Событие
   хранит все суставы упражнения с максимальной степенью нагрузки, а п.5
   считается по пересечению множеств (§8.4). Тай-брейк по порядку объявления
   снят: он был детерминированным, но клинически произвольным — канонический
   пример §6.2 (`hip_thrust_barbell`) даёт ничью `lower_back`/`hip`, и выбор
   первого молча обнулял бы счёт второго. Упорядочение степеней
   (`low` < `medium` < `high`) объявляется здесь явно: оно нужно, чтобы
   «максимальная степень» имела смысл, и раньше было выведено из §6.3/§14.4, а
   не записано. Схемная сторона закрыта вместе с продуктовой: `sets.pain_joints`
   (§3.1), факт фиксируется на момент события, переразметка контента задним
   числом уже случившиеся события не меняет. Что осталось за пределами решения:
   какой из суставов множества становится ограничением, решает пользовательница
   в диалоге (§14.4), а не приложение.
...
17. **Момент вызова `applyingClosedCycles` при синхронизации** (§11.5). Функция
    ...
    **На вебе вопрос острее, чем «когда»: там её не вызывает никто.**
    `cycle_events` пишет фронтенд напрямую (§20.6), сервер об отметке узнаёт
    только из запроса `/rebuild` с причиной `cycle_event_added` (§20.3), и
    `low_confidence_streak` не двигается ни в одном из путей. Значит
    автоматический выход из режима «без фаз» по низкой уверенности — единственная
    причина §11.5, которая вообще снимается сама, — на вебе не срабатывает.
    Закрывать вместе с основным вопросом: решение обязано назвать одно место
    вызова на обеих платформах, иначе веб и iOS разойдутся в режиме цикла у одной
    и той же пользовательницы.
...
19. **Предел размера дерева решений** (§20.9) — **закрыт: 512 узлов**, и этап В2
    он больше не блокирует. Число выбрано не замером, а из потолка, который уже
    стоит в §7.3: подходов на упражнение не больше пяти, значит переходов в
    дереве не больше четырёх, а верхняя граница 6⁴ = 1296 узлов достигается
    только если ни одна ветка не сойдётся — при округлении к лестнице
    достижимых (§9.5) такого не бывает. То есть предел в MVP не срабатывает ни
    при каком инвентаре, и 512 — это предохранитель, а не рабочий режим.
    Протокол обрезки задан в §20.9 и работает независимо от числа. Замер на
    первом срезе контента остаётся — но как подтверждение, а не как условие
    начала работы.
...
├── Packages/                    # общий Swift-код: FitCore, FitAPI, FitContent, FitData
├── Supabase/                    # миграции, RLS, edge functions
├── Tools/
└── Tests/
```
...
**Обратное направление живёт не здесь, а в `Packages/FitAPI`** (§2.1). Типы
FitCore наружу не отдаются: ни один из них не `Codable`, и делать их такими
нельзя — у enum с ассоциированными значениями (`ReasonCode` и всё, что в нём
вложено) синтезированная форма привязана к именам случаев в Swift, так что
переименование внутри ядра молча меняло бы контракт у двух развёрнутых
клиентов. `FitAPI` объявляет DTO обоих направлений явно, с ручными
`CodingKeys`, и конструирует их из значений FitCore.

Почему отдельный пакет, а не шестой подкаталог `Server/`: исходящая форма — это
та же половина соответствия, что и `Mapping/`, и ей нужна та же гарантия.
Положив её в сервер, мы получили бы ровно то, чего §20.2 избегает, — второй
клиент, пишущий вторую форму. Из `FitAPI` же берутся и TypeScript-типы
фронтенда, так что у границы `/v1` оказывается один источник на три стороны.

Там же живёт каталог русских формулировок к доменным причинам (§20.3): причина
та же, что и для DTO — iOS в офлайне обязан читать тот же каталог, а не писать
второй.

Разделение ответственности между тремя: `Mapping/` читает схему и собирает вход
FitCore, `FitCore` считает, `FitAPI` описывает, как результат выглядит на
проводе, и несёт формулировки к нему. Сервер не делает ничего из этих трёх дел
сам.

**И это не только отображение.** В нескольких местах `Mapping/` обязан
посчитать, а не переложить: подставить режим фаз пользователю без строки
`cycle_profiles`, перемножить вклад мышцы на стоимость утомления, обнулить
набор гирь при снятом флаге, различить утомление до распада и после. Полный
список — в §20.6; он часть контракта, а не деталь реализации, потому что тот же
модуль линкует второй клиент, и разойтись эти два клиента могут только здесь.
...
| `/v1/week-plans/{week_start}` | План недели: дни, статусы, состав каждого не начатого дня, строки статуса §7.1 |
| `/v1/week-plans/{week_start}/preview?date=&tz=` | Тот же план, срезанный по одной дате (см. ниже — это проекция, а не отдельный расчёт) |
...
| Путь | Делает | Тело |
|---|---|---|
| `POST /v1/week-plans` | Генерирует неделю §7.2 | `{week_start, date, tz}` |
| `PATCH /v1/week-plans/{week_start}/days/{day_id}` | Акцент, тип дня, отметка статуса | `{accent_muscle?, session_kind?, status?, date, tz}` |
| `POST /v1/workouts` | Начинает тренировку из `planned_day`, собирает состав §7.3 | `{planned_day_id, date, tz, equipment_profile_id?}` |
| `POST /v1/workouts/{id}/exercises/{we_id}/sets` | Логирует подход, отдаёт новое дерево §20.9 | `{id, set_index, prescribed_kg?, actual_kg?, actual_reps?, feedback?, pain_flag, rest_seconds?, skipped, completed_at, date, tz}` |

Действия (`POST` с глаголом):

| Путь | Делает | Тело |
|---|---|---|
| `POST /v1/week-plans/{week_start}/rebuild` | Пересборка §7.1; ответ содержит `ReasonCode` для «План обновлён» | `{cause, date, tz}` |
| `POST /v1/workouts/{id}/finish` | Закрывает тренировку, пересчитывает производные §20.6 | `{date, tz}` |
| `POST /v1/workouts/{id}/exercises/{we_id}/substitute` | Замена §13.4, включая перенос веса по `family_load_ratio` | `{to_slug, reason, date, tz}` |

`date` и `tz` есть в каждом теле не по инерции: §20.7 требует их всюду, где день
имеет значение, и сервер их валидирует до любой записи.

#### Что стоит за отдельными телами

**`POST /v1/week-plans` идемпотентен.** Повторный запрос на уже существующую
неделю возвращает её как есть и ничего не перегенерирует. Довод тот же, по
которому §20.6 завела клиентский `id` у подходов: офлайновой очереди на вебе нет
(§20.10), и повтор после таймаута — это просто второе нажатие кнопки. Ключ
идемпотентности здесь естественный и уже в схеме — `unique (user_id,
week_start)`, клиентского идентификатора не нужно. Перегенерация запрещена
отдельно: §7.1 не разрешает трогать неделю, которую пользовательница уже видела
и по которой, возможно, уже тренировалась. Принимаются только текущая и
ближайшая следующая неделя — прошлые не генерируются вовсе, а дальние не имеют
смысла, пока набор дней может смениться (§7.1).

**Генерация недели — два вызова FitCore, а не один.** `Planner.weekGrid` отдаёт
только пары (дата, тип): ни `id`, ни целевого вектора в них нет, а
`planRemainingDays` требует уже существующие `planned_days` и ничего не создаёт.
Поэтому последовательность такая: `weekGrid` → взять вектор пары (тип, акцент)
из `FitContent` → вставить строки `planned_days`, где `id` минтит сервер →
`planRemainingDays` → отдать. Всё это в одной транзакции, по общему правилу
§20.4: иначе вход планировщика — несогласованный срез.

**`PATCH .../days/{day_id}` — там, где §7.2 отдаёт решение пользователю.** Акцент
дню ставит пользователь, и тип тренировки он вправе заменить в любой день; у
`planned_days` есть строка в §3.1, значит по правилу границы выше это REST, а не
действие с глаголом. Тем же запросом ставится `status` — и это закрывает вторую
дыру: пропуск тренировки объявлен триггером пересборки (§7.1), а эндпоинта, из
которого берётся `skipped`, в §20.3 не было. Причину пересборки
(`RebuildCause.dayEdited`) проставляет сервер сам, по тому, что изменилось в
теле; клиент её не называет.

**`/rebuild` принимает ровно две причины: `equipment_changed` и
`cycle_event_added`.** Не потому, что остальные запрещены, а потому что остальные
серверу называть незачем: выполнение, пропуск, старт, оверрайд и правка дня — это
его собственные эндпоинты, и он наблюдает их сам. Две оставшиеся — единственные
события, которые происходят мимо него: `equipment_profiles` и `cycle_events`
фронтенд пишет напрямую (§20.6). По `cycle_event_added` сервер сам решает, что
это было — смена фазы или смена уверенности, — сравнив состояние цикла до и
после; клиент такого различения не делает. Отдельной «ручной пересборки» нет:
восьмого случая в `RebuildCause` не заводится, потому что у кнопки «пересобрать»
нет причины, которую можно было бы напечатать в строке «План обновлён» (§7.1), а
молчаливая пересборка запрещена там же.

**`POST /v1/workouts` называет профиль инвентаря.** `workouts.equipment_profile_id`
есть в §3.1, и §13.1 держит несколько профилей («Дом», «Зал у работы») —
значит выбор принадлежит моменту, а не аккаунту. Поле необязательное; без него
берётся `is_default`.

**Момент оценки утомления на старте — разный для состава и для весов.** §7.1
фиксирует 12:00 для сегодняшнего и будущих дней, и довод там — идемпотентность:
пересборка в 9:00 и в 18:00 дала бы разные тренировки. К начатой тренировке этот
довод неприменим — её не пересобирают никогда («правило неприкосновенности»), —
и оба варианта, взятые целиком, платят слишком дорого: по 12:00 вечерняя
тренировка считает утомление на несколько часов старее и занижает веса (семь
часов при полураспаде 20 ч — это около 21%, а пороги §8.2 стоят на 0.8 и 1.8),
а «сейчас» меняет состав между карточкой дня и нажатием «Начать», чего §7.1 не
разрешает: молчаливое изменение читается как баг, а объяснить его нечем — «прошло
время» причиной не является.

Поэтому момент разводится по роли числа:

| Что | На какой момент считается |
|---|---|
| Состав упражнений, порядок, `target_sets` | 12:00 дня (§7.1) — совпадает с тем, что показала карточка |
| `weight_readiness`, `prescribed_kg`, `target_rir` | Фактический момент старта |

**Реализуется это вторым вызовом поверх уже построенного состава, а не вторым
полем утомления.** `SessionInput.fatigue` остаётся одним словарём с одним
смыслом («уже с распадом до момента оценки»); сборка вызывается на 12:00, её
результат фиксирует состав, после чего числа упражнения пересчитываются на
утомлении, продеканном до момента старта. Держать два представления одного и
того же входа дороже, чем вызвать пересчёт отдельно: два поля пришлось бы
объяснять каждому, кто читает контракт, и первая же ошибка в том, какое из них
куда идёт, была бы невидимой — ровно того рода, о котором предупреждает §20.6
(«утомление в двух входах означает разное»).

Вызов этот — **`Planner.prescribe`** (FitCore): принимает уже собранную сессию и
утомление на нужный момент, оставляет состав, порядок и `target_sets` как есть и
пересчитывает только `target_rir`, `weight_readiness` и `prescribed_kg`. Второй
`buildSession` на продеканном утомлении эту роль не выполняет и выполнить не
может: утомление входит в ПОДБОР четырьмя путями — множитель объёма мышцы,
слагаемое w5, выбор упражнения под ±1 подход на сессию (§10) и надбавка RIR, —
так что второй полный вызов сменил бы и состав, и раскладку подходов, то есть
ровно то, что этот абзац запрещает.

Функция живёт в ядре, а не в `Server/`, по правилу §20.1 и по тому же доводу,
что замена упражнения (§13.4): тренировка начинается не в 12:00 и на iOS, где
§4.3 требует работы без сети, — логика, оставленная на сервере, там недоступна.
Публичной частью FitCore она становится вместе с этим решением; пересчитываемых
величин ровно три, и все три считаются из готовности и утомления, а дневная
готовность §10 от утомления не зависит вовсе.

`target_rir` попадает во вторую группу не по аналогии, а вынужденно: §10 связывает
вес с порогом RIR — надбавка готовности выше 1.0 срезается ровно тогда, когда
утомление подняло RIR. Пересчитать вес на одном утомлении, а RIR оставить на
другом значило бы разорвать эту связь и выдать вес, не соответствующий
собственному порогу.

**`/preview` — проекция, а не отдельный расчёт.** Заморозка состояния будущего
дня (§18, 31b) уже реализована внутри `planRemainingDays`: готовность считается
от состояния цикла на дату дня без check-in и оверрайда, утомление — с распадом
до момента оценки этого дня. Отдельной функции предпросмотра одного дня в FitCore
нет и заводить её незачем; эндпоинт существует только чтобы клиент мог спросить
один день, не забирая неделю целиком.

**`prescribed_kg` в теле подхода приходит от клиента, `prescribed_reps` — нет.**
Асимметрия следует из того, кто что знает. Вес подходов со второго назначает
дерево §20.9, применённое во фронтенде: сервер этого числа не видел, а
восстановить его может только проигрыванием дерева — и любой ввод, уводящий
состояние с дерева (флаг боли, ручная правка веса или повторов, замена,
пропуск подхода), проигрывание обрывает; §20.9 перечисляет эти вводы прямо.
Поэтому вес приезжает в теле. У повторов такого довода нет: `prescribed_reps` —
верх действующего диапазона, он уже лежит в `workout_exercises.target_rep_max`,
записанном сервером при старте, и второй источник завёл бы расхождение там, где
его нечем обнаружить (§20.6: колонку не читает ни одно правило).

**Снимок `week_plans.last_shown_plan` (§20.6) `/preview` не пишет** — ни целиком,
ни частью. Срез по одной дате, положенный в снимок, заставил бы следующую
пересборку счесть исчезнувшими все остальные дни и напечатать «План обновлён»
там, где ничего не менялось.

#### Форма ответа

**Успех — голое тело, без конверта.** `{"data": …}` рассматривался и отклонён по
одной причине: `/v1/content/*` обязан отдавать ровно те байты, от которых
считается `ETag` (§20.11), а конверт добавил бы к ним слой, меняющийся вместе с
версией сервера. Ветку «успех или ошибка» клиент берёт из HTTP-статуса.

**Ошибка — всегда**
`{"error": {"code": "...", "message": "...", "details": {...}}}`. Роли полей
разведены, и это часть контракта:

- `code` — машинный, из закрытого словаря ниже; по нему клиент ветвится;
- `details` — машинный и открытый: путь к невалидному полю, конфликтующий `id`,
  присланная и серверная даты. Пользовательнице он не показывается никогда;
- `message` — человекочитаемое поле, готовая русская формулировка (общее правило
  — в «Причинах на проводе» ниже).

Закрытый словарь `code`:

| Код | HTTP | Когда |
|---|---|---|
| `unauthorized` | 401 | Подпись, `exp`, `iss`/`aud` (§20.5, тест 20b) |
| `jwks_unavailable` | 503 | Ключей нет и кеша нет (§20.5) |
| `forbidden` | 403 | RLS не отдала строку, которая обязана быть |
| `unlinked_account` | 403 | Анонимный аккаунт на пути, требующем линковки (§20.5) |
| `not_found` | 404 | |
| `date_skew` | 422 | Присланная дата расходится больше чем на сутки (§20.7, тест 20d) |
| `validation_failed` | 422 | Тело не проходит схему; поле — в `details` |
| `week_exists` | 409 | Неделя уже есть и перегенерации не подлежит |
| `set_id_required` | 422 | Тело подхода без `id` (§20.6, тест 20e) |
| `set_immutable` | 409 | Изменение подхода с непустым `completed_at` (§4.3, тест 20f) |
| `workout_not_started` | 409 | Запись в тренировку, которой нет |
| `workout_finished` | 409 | Запись в закрытую тренировку |
| `day_not_planned` | 409 | Старт по дню со статусом `done`/`skipped`/`replaced` |
| `generator_disabled` | 409 | §14.3, беременность |
| `internal` | 500 | |

Словарь закрыт: новый код добавляется правкой этого раздела, а не решением в
роуте. Признак, по которому код заводится, — различимое поведение клиента, а не
различимая причина в логе: два случая, на которые фронтенд реагирует одинаково,
получают один код и разные `details`.

**Доменные причины в конверт ошибки не попадают.** Пустая тренировка,
ослабленный минимум паттернов, недобор по времени — это успешный ответ с
`ReasonCode`, а не ошибка.

#### Тела успешных ответов

Форма каждого тела — часть контракта, живёт в `FitAPI` (§20.2) и роутом не
выбирается. Конверта ни у одного нет (см. выше).

| Путь | Тело |
|---|---|
| `GET /v1/today` | `TodayCard` |
| `GET /v1/week-plans/{week_start}` | `WeekPlanResponse` |
| `GET /v1/week-plans/{week_start}/preview` | `WeekPlanResponse` |
| `GET /v1/workouts/{id}` | `Workout` |
| `GET /v1/workouts/{id}/exercises/{we_id}/alternatives` | `[ExerciseAlternative]` |
| `GET /v1/content/exercises`, `/v1/content/stretches` | байты пакета, с `ETag` (§20.11) |
| `POST /v1/week-plans` | `WeekPlanResponse` |
| `PATCH /v1/week-plans/{week_start}/days/{day_id}` | `WeekPlanResponse` |
| `POST /v1/week-plans/{week_start}/rebuild` | `WeekPlanResponse` |
| `POST /v1/workouts` | `Workout` |
| `POST /v1/workouts/{id}/exercises/{we_id}/sets` | `{decision_tree}` |
| `POST /v1/workouts/{id}/finish` | `Workout` |
| `POST /v1/workouts/{id}/exercises/{we_id}/substitute` | `WorkoutExercise` |

Поля тел, которых раньше не было:

```
TodayCard         {date, readiness, cycle: CycleState, day: DayOutcome,
                   reasons: [Reason]}
WeekPlanResponse  {plan: {days: [DayOutcome], status_lines: [Reason]},
                   notice: Reason?}
Workout           {id, planned_day_id?, session_kind, accent_muscle?,
                   started_at, finished_at?, readiness, cycle_phase?,
                   cycle_confidence?, is_calibration,
                   exercises: [WorkoutExercise]}
WorkoutExercise   {id, slug, order_index, target_sets, target_rep_min,
                   target_rep_max, target_rir, prescribed_kg?,
                   weight_readiness, is_calibration, substituted_from?,
                   substitution_reason?, decision_tree?, sets: [Set]}
Set               {id, set_index, prescribed_kg?, prescribed_reps, actual_kg?,
                   actual_reps?, feedback?, pain_flag, pain_joints,
                   rest_seconds?, completed_at?, skipped}
```

**Четыре недельных пути отдают одно тело.** `WeekPlanResponse` один на `GET`,
генерацию, правку дня и пересборку: у всех четырёх ответ — это неделя, и
различаются они только тем, есть ли что сказать про изменение. `notice` — та
самая строка «План обновлён» (§7.1); `null` там, где сравнивать не с чем (первая
генерация) или где ничего не изменилось. Один разбор на четыре пути дешевле
четырёх похожих.

**Дерево — поле упражнения, а не отдельная карта.** `decision_tree` лежит внутри
`WorkoutExercise`: строится оно от веса и остатка подходов ИМЕННО этого
упражнения, и параллельная карта `we_id → дерево` заставляла бы клиента сшивать
две структуры по ключу ради связи, которая и так однозначна. Поле необязательное
и приезжает только у начатой тренировки: у не начатого дня дерева не существует
— ни стартового веса на момент старта, ни числа оставшихся подходов ещё нет.
Поэтому в `WeekPlanResponse` и `TodayCard` его нет вовсе.

**`POST …/sets` отдаёт ТОЛЬКО новое дерево, а не готовый следующий подход.**
Клиент уже применил ветку мгновенно — ради этого §20.9 и существует, — и готовый
подход в ответе стал бы вторым источником того же числа. Разойдясь с уже
показанным, он дал бы худшее из возможного: цифра меняется после того, как
пользовательница её увидела и приняла. Досрочное завершение упражнения (§9.3)
клиент узнаёт оттуда же — из узла с `terminates`.

**`/substitute` отдаёт одно упражнение, а `/finish` — тренировку.** Замена меняет
ровно одну строку `workout_exercises` (новый слаг, перенесённая базовая линия
§13.4, новое дерево), и отдавать целую тренировку значило бы заставить клиент
перерисовать экран, на котором идёт подход. Завершение, наоборот, меняет
тренировку целиком и пересчитывает производные (§20.6), и её итог — это то, что
показывает экран завершения.

#### Причины на проводе

`ReasonCode` едет тремя полями: `code`, `params` и `message`.

```json
{
  "code": "phase_periodization",
  "params": { "phase": "late_luteal", "cycle_confidence": 0.62 },
  "message": "Многие отмечают спад в этой фазе — предлагаем работать чуть легче"
}
```

**`params` — объект с именованными полями своего случая, а не безымянный
мешок.** Форма у каждого кода своя и зафиксирована: у `planned_volume_loss` это
`{muscle, sets, cause}`, у `pattern_minimum_relaxed_by_limit` —
`{fitted, limit}`, у `day_vector_missing` — `{kind, accent}`. Вложенная причина
остаётся вложенной: у `plan_rebuilt` в `params` лежит `cause` — такой же объект
из `code`/`params`. Во фронтенде это размеченное объединение по `code`, то есть
поля типизируются так же, как при плоской форме, но конверт у всех причин один
и разбирается одним куском кода.

Требование §14.6 «неопределённость должна быть машиночитаемой» выполняется
`params`: `cycle_confidence` лежит там у **всякой** причины фазового
происхождения, включая вложенные в `plan_rebuilt` `phase_changed` и
`cycle_confidence_changed`, а не только у тех, что оказались верхним уровнем
ответа. Клиент обязан этим значением воспользоваться — предъявить рекомендацию
тем осторожнее, чем ниже уверенность, — и `message` его от этой обязанности не
освобождает.

**Формулировку отдаёт сервер, для всех доменных причин, а не только в конверте
ошибки.** Решающий довод — согласованность двух платформ и цена правки тона:
§13.5 и §14.6 задают, как приложение разговаривает о теле пользовательницы, и
каталог строк, размноженный по вебу и iOS, расходится тихо, а чинится релизом в
App Store. Прецедент прямой и уже оплачен: §20.11 приняла, что «правка одной
опечатки в cue требует деплоя сервера», — формулировки причин из того же
теста.

Это отступление от правила `FitCore`, где `ReasonCode` несёт код, а русские
строки живут в `App/FitTrack/Localization`, и отступление намеренное. Граница
между ними проходит по тому, кто спрашивает: `FitCore` вызывают оба клиента
напрямую, и текста у него быть не может; `/v1` — это уже ответ сервера, у
которого каталог строк один на всех. Клиент вправе `message` не показывать и
собрать свою строку из `code` и `params` — контракт этого не запрещает и на iOS
в офлайне именно так и придётся, — но по умолчанию он показывает то, что
пришло.

Тон §13.5 и §14.6 действует на этот канал наравне с экранными, и теперь
буквально: строки, которые видит пользовательница, приезжают отсюда.

**Итог дня едет в той же форме.** `DayOutcome.cause` — доменная причина, которую
видит пользовательница («день пропущен», «заменён растяжкой»), поэтому она едет
объектом `{code, params, message}`, как всякая другая, а не голой строкой. Иначе
формулировку собирал бы клиент — ровно то, от чего абзац выше увёл все остальные
причины, и ровно там, где двум платформам разойтись легче всего.

`params` у всех девяти случаев пуст, и сплющивать из-за этого форму не надо:
один конверт разбирается одним куском кода на все причины сразу, а появившийся
однажды параметр не сменит форму ответа.

Словарь `cause` закрыт, девять значений: `rest_override`, `skipped`, `replaced`,
`started`, `done`, `past`, `grid_stretch`, `markup_missing`, `pregnancy`.

**`kind` остаётся строкой и тоже закрыт** — пять значений: `session`,
`stretching`, `not_built`, `vector_missing`, `generator_disabled`. Он не причина,
а ветка показа: по нему клиент выбирает, что рисовать, и формулировки у него
нет. Перечислен здесь по тому же правилу, что словарь кодов ошибок: новое
значение добавляется правкой этого раздела, а не случаем в enum.
...
Уровень задаётся отдельным утверждением, а не в `begin`, потому что
`PostgresClient.withTransaction` шлёт голый `BEGIN;`. Разницы в смысле нет —
`set transaction` действует, пока в транзакции не выполнено ни одного запроса, —
а форма здесь ровно та, что в коде.

**Одним утверждением, а не двумя `set local`.** Причин две, и каждой хватило бы
самой по себе: `SET` не принимает привязанный параметр, а claims обязаны ехать
именно им (§20.5), и одно утверждение вместо двух снимает round-trip с
критического пути каждого логируемого подхода (§19.1). Эквивалентность
`set_config('role', …, true)` и `set local role authenticated` замерена на живой
базе ДО того, как записана сюда: обе дают `current_user = authenticated`, обе
снимаются на `commit`, и `auth.uid()` после совмещённого утверждения читается.
...
**Пул логинится ролью `fitserver`, у которой нет собственных прав.** Роль
заводится миграцией `0008` как `login noinherit` с единственным членством —
`authenticated`, — то есть не может прочитать ни одной таблицы, пока не станет
`authenticated` подстановкой выше. Это не запас прочности, а единственное, что
делает изоляцию свойством базы, а не дисциплины кода: без этого забытая
подстановка в одном пути давала бы не отказ, а молчаливый доступ ко всем строкам
всех пользователей, и ни один тест §20.15 этого не поймал бы — оба запроса 20a
идут правильным путём.

Соблазн войти под `postgres` и положиться на RLS не работает, и замер объясняет
почему точнее, чем ожидание: у роли `postgres` в Supabase `rolbypassrls = true`.
Она обходит RLS по атрибуту роли, а не по владению таблицами, поэтому
`force row level security` её бы не остановил — этот путь закрыт не «дороже», а
целиком. Обратная сторона `noinherit` замерена там же: забытая подстановка даёт
`permission denied for table profiles` на первом же чтении.

`fitserver` остаётся ровно настолько же бесправной, насколько `authenticated`:
подстановка не повышает права, а даёт ровно те, что есть у запроса из браузера.
Правило «второй модели доступа не заводится» этим не нарушается, а исполняется.

**Уровень изоляции — `repeatable read`, и это не перестраховка.** Обещание
абзаца про `SessionInput` ниже — согласованный срез входа — транзакцией по
умолчанию не выполняется: в `read committed` каждое утверждение берёт новый
снимок, и транзакция, читающая утомление первым запросом, а `exercise_states`
пятым, даёт ровно тот разъезжающийся вход, который абзац объявляет невозможным.
Одна транзакция сама по себе даёт атомарность записей и общий контекст claims,
но не общий снимок чтения.

Цена — ошибки сериализации (`40001`) на пишущих путях. Правило: **один
прозрачный повтор всей транзакции**, и только на этот класс ошибки. Повтор
безопасен потому, что откат при `repeatable read` полон — не остаётся ни одной
записи, — и повторный проход перечитывает вход заново. Второго повтора нет:
если конфликт воспроизвёлся, это не гонка двух вкладок, а что-то, о чём клиент
должен узнать.

**Параметры пула** (умолчания `postgres-nio`, где не сказано иное):

| Параметр | Значение | Почему |
|---|---|---|
| `maximumConnections` | 20 | при правиле «одно соединение на запрос» (§19.1) это и есть потолок конкурентности сервера |
| `minimumConnections` | 2 | §20.13 исключил серверлесс из-за холодного старта пула; ноль возвращал бы его после каждого затишья |
| `connectTimeout` | 5 с | столько же, сколько у похода за JWKS (§20.5): дольше ждать на критическом пути логирования подхода незачем |
| `connectionIdleTimeout` | 60 с | умолчание |
| `requireBackendKeyData` | `true` | умолчание; снимается только ради прокси вроде RDS Proxy, а мы ходим напрямую |

**Подключение прямое, а не через пулер Supabase.** Транзакционный пулер перед
нашим собственным пулом означал бы два пула подряд и отменил бы долгоживущее
соединение, ради которого §20.13 отказался от серверлесса. TLS обязателен в
проде и отсутствует локально — это разница конфигурации, а не кода.

**Точка входа в транзакцию одна, и она же проверяет токен.** `DB/` принимает
заголовок `Authorization` и зовёт `Auth/` сам, а не получает уже проверенный
токен снаружи. Иначе порядок «сначала проверка, потом БД» был бы свойством слоя
роутов, то есть дисциплиной, которую новый роут нарушает молча; здесь открыть
транзакцию без проверенного токена нечем. Тем же выбором становится проверяемой
вторая половина 20b (§20.15): «до БД запрос не доходит» — утверждение о том, что
соединение из пула не арендовано ни разу, и наблюдать его можно только там, где
аренда происходит.
...
подписью и запрос с истёкшим токеном. Это тесты 20a и 20b, и оба требуют
эндпоинтов, то есть `Mapping/` и непустого `FitContent`.

**До появления эндпоинтов изоляция закрывается тестом 20k, и он 20a не
заменяет.** 20k прогоняет тот же сценарий через настоящую точку входа `DB/` с
двумя настоящими JWT против живой базы: настоящий пул, настоящая роль
`fitserver`, настоящий `set_config`, настоящая RLS — всё, кроме HTTP и роутов.
Отдельный номер нужен именно затем, чтобы 20a нельзя было отметить сделанным:
20k ничего не говорит о том, ходит ли по этому пути хоть один роут, а 20a только
об этом и говорит.

Чтение всего входа `SessionInput` идёт **одной транзакцией и в `repeatable
read`**. Иначе вход — несогласованный срез: утомление посчитано до записи
тренировки, `exercise_states` после. Одной транзакции для этого мало, см. выше.
...
**Допустимые алгоритмы — `ES256` и `RS256`, список закрытый.** Оба — то, чем
Supabase подписывает сегодня, и список закрыт по тому же правилу, что словарь
кодов §20.3: новый алгоритм появляется правкой этого раздела, а не обновлением
зависимости. Обе позиции обязательны: ES256 создаётся по умолчанию, но ключ
RS256 заводится в дашборде одним действием, и список из одного алгоритма
превратил бы это действие в событие, требующее деплоя, — против обещания
абзацем выше.

**Алгоритм берётся из ключа, найденного по `kid`, а не из заголовка токена.**
Заголовок не подписан, и доверие к нему — обход проверки целиком, а не её
ослабление. Отсюда три правила, каждое с явным утверждением в 20b, а не «по
умолчанию библиотеки»: `alg: none` отклоняется; несовпадение заголовка с
алгоритмом найденного ключа отклоняется; симметричные алгоритмы отклоняются по
типу ключа, а не сравнением строк — иначе HS256, подписанный байтами публичного
ключа, остаётся живым. Требовать здесь явные утверждения нужно по той же
причине, по которой 20b закрывается `iss`, а не промахом `kid`: зелёный
обязательный тест обязан быть зелёным по нашей проверке.

**Какие claims проверяются:**

| Claim | Правило |
|---|---|
| `exp` | Обязателен; истёкший токен — 401 |
| `nbf` | Если есть — не в будущем |
| `sub` | Обязателен; он и есть `auth.uid()` — уходит в `request.jwt.claims` (§20.4) |
| `iss` | Обязан совпасть с issuer проекта |
| `aud` | Обязан быть `authenticated` |

**Тест 20b («токен другого проекта») закрывается `iss`, а не `aud`.** Это стоит
сказать прямо, потому что выглядит наоборот: `aud` у Supabase равен
`authenticated` во всех проектах сразу и чужой токен не отличает. Промах по
`kid` чужой токен тоже отсечёт — ключа не найдётся, — но опираться на это одно
значило бы держать обязательный тест зелёным по случайной причине, а не по
проверке.

**`role` из токена игнорируется.** Сервер ставит `set local role authenticated`
безусловно (§20.4), что бы ни лежало в claim. Иначе правило «`service_role` на
сервере не живёт» держалось бы на том, что никто не пришлёт токен с другим
`role`.

**Промах по `kid` — один перезапрос JWKS, с двумя счётчиками сразу.** Без
перезапроса ротация ключей давала бы окно сплошных 401 у всех разом, посреди
тренировок, — а §20.5 продаёт асимметрию именно тем, что ротация проходит без
деплоя. Троттл нужен, потому что перезапрос инициирует неаутентифицированный
запрос: без ограничения это внешний рычаг на исходящий трафик сервера.

Счётчика два, и каждый закрывает то, чего не закрывает другой:

- **по `kid`, 60 секунд** — за одним и тем же неизвестным ключом второй раз не
  ходим; повторный промах по нему — 401 без похода;
- **глобальный, 60 секунд на процесс** — сходить можно не чаще раза в минуту,
  сколько бы разных `kid` ни пришло.

Одного учёта по `kid` недостаточно, и это не запас прочности: `kid` выбирает
тот, кто прислал токен, это UUID в неподписанном заголовке, и N разных
неизвестных значений дают N исходящих запросов в минуту — ровно тот рычаг, ради
которого троттл и введён. Закрыть его больше нечем: подпись до похода за ключом
не проверить (ключ для этого и нужен), а `iss`, `aud` и `exp` не подписаны и
подставляются любые. Одного глобального счётчика хватает для трафика, но он
заставлял бы настоящий новый `kid` ждать до минуты из-за чужого промаха. Вместе:
по `kid` решается, стоит ли ключ похода, глобальным — можно ли идти сейчас.

**JWKS недоступен, а кеш есть — работаем по кешу.** Ключ не протух от того, что
Supabase недоступен, и отказ означал бы остановку тренировок у всех
пользователей из-за чужого сбоя. Кеша нет вовсе (холодный старт сервера во время
сбоя) — `jwks_unavailable` и 503, а не 401: это не «токен плохой», и клиенту
следует повторить, а не разлогинивать пользователя.

**Кеш обновляется каждые 10 минут, первый поход — при старте процесса.**
Число не выбрано на глаз: у JWKS-эндпоинта Supabase примерно столько же кеша на
периферии (`Cache-Control: max-age=600` в ответе), то есть чаще ходить незачем —
свежее не отдадут, — а реже значило бы добавлять свою задержку к чужой. Срок
здесь измеряет ровно одно: сколько мы продолжаем доверять отозванному ключу.
Отзыв у Supabase немедленный и обязан отклонять уже выписанные токены, так что
«час всё равно живёт токен» здесь не довод. При живом сроке ротация промаха по
`kid` не даёт вовсе — новый ключ публикуется в наборе до того, как им начинают
подписывать, — поэтому механика перезапроса выше остаётся страховкой, а не
основным путём.

**Пустой список ключей при старте — процесс не поднимается.** Ответ `200` с
`{"keys": []}` отдаёт проект, не переведённый на асимметричные ключи. По коду
ответа это состояние от чужого сбоя неотличимо, но по самому ответу — вполне:
недоступность приходит таймаутом или 5xx, а пустой набор приезжает успехом.
Разница решающая, потому что сбой пройдёт сам, а конфигурация — нет. Поэтому
недоступность старту не мешает (процесс поднимется и будет отвечать
`jwks_unavailable`, пока ключи не приедут), а пустой набор его останавливает:
деплой с неработающей аутентификацией не должен прожить ни минуты, тогда как 503
на каждый запрос выглядел бы чужой бедой и жил бы до тех пор, пока кто-нибудь не
прочитает логи.

**Набор опустел в работе — `internal` и 500.** Это уже не старт: ключи были,
значит проект настроен, и внезапно пустой набор означает поломку у нас или в
проекте, а не временную недоступность. `jwks_unavailable` здесь соврал бы
дважды — обещал бы клиенту, что повтор поможет, и смешал бы нашу поломку с
чужим сбоем. За 503 остаётся то, что §20.3 за ним и закрепила: ключей нет и
кеша нет.

**Обновление заменяет набор ключей целиком.** Объединять старый набор с новым
выглядит безопаснее — ни один живой токен не отвалится, — но тогда отозванный
ключ не покидает кеш никогда, и отзыв перестаёт значить что-либо. Исключение
одно и уже названо: при недоступности JWKS сохраняется прежний набор.

**Допустимый перекос часов для `exp`/`nbf` — 60 секунд.** Масштаб здесь другой,
чем у суточного допуска §20.7, и путать их не надо: там сверяются часы телефона
с сервером, тут — часы сервера с Supabase, обе машины по NTP. Минута покрывает
сетевой разброс и не покрывает ничего больше.

**Анонимность — не свойство токена, а состояние аккаунта.** `is_anonymous` есть
в каждом токене Supabase, и токен анонимного пользователя полностью валиден:
§4.1 требует, чтобы онбординг и PAR-Q проходились анонимно, а `profiles` и
`cycle_profiles` пишет только сервер (§20.6) — серверный путь записи анонимному
пользователю нужен работающим. Поэтому claim в проверке подлинности не
участвует и 401 не даёт никогда.

**Линковку проверяет сервер, а не только кнопка.** На путях, требующих
привязанного аккаунта, токен с `is_anonymous = true` получает `unlinked_account`
и 403 (§20.3). Сегодня такой путь один — `POST /v1/workouts`, по букве
требования «кнопка „Начать тренировку“ требует привязанного аккаунта»;
генерация недели, профиль и инвентарь анонимному пользователю доступны, потому
что защищается потерянная история тренировок, а не план. Проверка живёт на
сервере по той же причине, по которой там живёт каталог формулировок (§20.3):
правило, оставленное интерфейсу, придётся написать второй раз в iOS-клиенте, а
обойти его можно запросом мимо интерфейса — и создастся ровно та история, ради
сохранности которой гейт введён. Код отдельный, а не `forbidden`: фронтенд на
него показывает экран линковки, и это различимое поведение клиента — признак, по
которому §20.3 велит заводить код. Статус 403, а не 409: это не конфликт
состояния запроса, а недостаточность прав предъявленного аккаунта.

**Issuer — одно значение на процесс, из конфигурации, без значения по
умолчанию.** Список issuer'ов не заводится: он существовал бы только чтобы
принимать токены двух проектов сразу, а `iss` — то, на чём стоит 20b, и одна
дописанная строка открыла бы его молча. Локальная среда отличается значением
(`http://127.0.0.1:54321/auth/v1` против `https://<ref>.supabase.co/auth/v1`), а
не формой. Незаданное значение — отказ при старте: issuer по умолчанию
относится к тому же классу молчаливой ошибки, что и проверка подписи ключами не
того проекта. URL JWKS отдельно не настраивается, а выводится как
`<issuer>/.well-known/jwks.json` — у Supabase это соотношение выполняется и
локально, и в проде, а раздельная настройка допускает расхождение, при котором
подписи проверяются ключами одного проекта, а `iss` — против другого.

**Дальше в транзакцию уходит полезная нагрузка дословно.** `set local
request.jwt.claims` (§20.4) получает расшифрованный сегмент проверенного токена
как он есть, привязанным параметром (`select set_config('request.jwt.claims',
$1, true)`: в `SET` параметр не биндится). Пересобирать JSON из разобранной
структуры нельзя — она отбросит незнакомые claims, и первая же политика или
триггер, прочитавший `auth.jwt() ->> 'email'`, получит `null` молча, без отказа.
Дословная передача заодно делает буквальным обещание §20.4: у запроса из
браузера в этой настройке лежат полные claims, и пути не расходятся ни в чём.
`role` в нагрузке присутствует и остаётся неисполненным — роль ставит
безусловный `set local role authenticated`.
...
**В `sets.prescribed_reps` пишется верх действующего диапазона** — то есть
`Planner.repRange(goal:state:).upperBound`, тот же источник, что у
`workout_exercises.target_rep_max` и у дерева §20.9 (§9.1). `PrescribedExercise`
числа на конкретный подход не несёт, и его там неоткуда взять: двойная
прогрессия §9.1 предписывает диапазон, а не число. Верх выбран потому, что
именно он — цель: §9.1 растит вес после того, как повторы добраны до верхней
границы, и «сколько предлагаем сделать» означает её. Колонку при этом не читает
ни одно правило — §9.3 сравнивает фактические повторы с границами диапазона, а
не с ней, — так что живёт она для показа и истории, и ошибка в ней не сдвинет
веса, а исказит то, что пользовательница видит в журнале.

**`sets.prescribed_kg` сервер, наоборот, принимает от клиента** (§20.3): вес
второго и дальнейших подходов назначило дерево §20.9 во фронтенде, и сервер его
не видел. Правило «доменные строки пишет только сервер» этим не нарушается — по
тому же разбору, что и клиентский `id` ниже: клиент сообщает факт, а решение о
записи, проверка инвариантов §4.3 и пересчёт производных остаются на сервере.
...
**Производные пересчитываются, а не мержатся** (§4.3) — но двумя разными
способами, и обещать здесь симметрию нельзя:

- `exercise_states` — свёртка `Progression.rebuildStates` по журналу подходов
  упражнения: один вызов, полная история на входе.
- `muscle_fatigue` — последовательное применение `Recovery.applying`, по вызову
  на момент времени. Это не свёртка: сама функция инкрементальна по построению,
  потому что экспоненциальный распад коммутативен по времени и достаточно хранить
  одно значение с меткой на мышцу — ровно то, что и делает таблица (§3.1).
  Пересчёт из журнала с нуля возможен и даёт тот же ответ, но это N вызовов, а не
  один вроде `rebuildStates`.

Общее у них то, ради чего правило и существует: обе величины выводятся из
журнала и никогда не мержатся построчно.
...
инкрементальным pull без единой правки §4.3. Вставка подхода двигает отметку
родительской `workouts` сама по себе — отдельного «изменения» для этого не
требуется. Идемпотентный ретрай (`upsert` тем же `id` и теми же значениями)
курсор **не** двигает: триггер обновления снабжён условием
`old.* is distinct from new.*` (§3.1), иначе повтор запроса выдавал бы pull
страницу, в которой ничего не изменилось.

**`week_plans.last_shown_plan` пишет сервер при каждой отдаче плана наружу.**
Это снимок того плана, который пользовательница действительно увидела, и нужен
он одному потребителю: `Planner.rebuildNotice` сравнивает два ВЫХОДА
планировщика и без предыдущего решить, печатать ли «План обновлён» (§7.1), не
может. Хранить состав будущего дня больше негде — строки `workouts` у него нет,
а `workout_exercises` висит на ней, — и пересчитать «как было» тоже нельзя: два
триггера из восьми уничтожают свой прежний вход. Выполненная тренировка
меняет утомление и `неделя[m]` необратимо, а старого инвентаря после правки
`equipment_profiles` в базе просто не остаётся (эту таблицу пишет фронтенд
напрямую). Без снимка строка «План обновлён» для этих двух случаев была бы
невычислима, а §7.1 называет молчаливую пересборку недопустимой.

Снимок не заменяет `input_digest` и не пересекается с ним по назначению: тот
хранит ВХОД и служит отладке, этот — ВЫХОД и участвует в решении, что показать.
`null` означает «сгенерирована, но ни разу не показана»: сравнивать не с чем,
строка не печатается.

**Форма снимка — своя и узкая, а не форма ответа.** В нём лежит ровно то, что
читает `Planner.isPlanChange`: на каждый день — `kind`, `cause` и состав
(слаг → `target_sets`).

```json
{"v": 1,
 "days": {"<day_id>": {"kind": "session", "cause": null,
                       "composition": {"<slug>": 3}}}}
```

`WeekPlanDTO` на эту роль не берётся, хотя выглядит готовым: он несёт
`estimated_seconds`, `reasons` и `status_lines`, которых сравнение не читает, и
привязал бы формат ХРАНЕНИЯ к форме ОТВЕТА — правка §20.3 стала бы правкой
данных у всех живых пользователей. Тип живёт в `FitAPI` рядом с `WeekPlanDTO` и,
в отличие от него, обязан читаться ОБРАТНО в значения FitCore: без этого
`rebuildNotice` вызвать нечем, а тот же код понадобится iOS, где пересборка та
же.

**Номер версии обязателен с первой записи** — правило то же, что у
`input_digest`, а причина сильнее: дайджест читают глазами при отладке, снимок
читают кодом. Строка версии, которую прочитать нечем, равносильна `null`: одна
пересборка проходит молча, и это лучше, чем разобранная неверно.

**Пишут снимок только пути, отдающие неделю целиком:**
`GET /v1/week-plans/{week_start}`, `POST /v1/week-plans` и
`POST /v1/week-plans/{week_start}/rebuild`. `/preview` не пишет никогда (§20.3).
Цена принята явно: пользовательница, смотревшая только карточку дня,
сравнивается с тем, что показывала неделя, — то есть «действительно увидела»
здесь читается как «увидела последний полный показ». Обратное, снимок из
одного дня, ломает сравнение целиком, а не приближает его.

Отсюда следствие, которое стоит назвать прямо: **`GET` недельного плана —
пишущий запрос.** Границу стилей §20.3 это не нарушает (ресурс тот же), но
транзакция у него та же, что у записи, со всеми правилами §20.4.

**Порядок внутри `/rebuild` несущий: сначала сравнить, потом перезаписать.**
Переставленный местами он даёт молчание всегда — снимок сравнился бы с только
что посчитанным планом сам с собой.

**`week_plans.is_deload` и `input_digest` пишет сервер в момент генерации.**
`is_deload` — признак разгрузочной недели (§11.5), зафиксированный на эту
неделю и не пересчитываемый пересборкой; пока правило счёта открыто (§19.2 п.4),
там всегда `false`. `input_digest` — полный снимок входа генерации с номером
версии (`{"v": 1, ...}`). Его назначение — отладка, поддержка и
воспроизводимость: «какой именно вход дал этот план». Причину пересборки он не
определяет и определять не может — строку «План обновлён» §7.1 даёт
`Planner.rebuildNotice`, а она сравнивает два выхода планировщика и дайджеста не
читает.

**Моменты времени сервер приводит к шкале FitCore один раз — при записи**, а не
при каждом чтении (§20.7). Колонки, которые хранят значения этой шкалы
(`muscle_fatigue.updated_at`, `exercise_states.last_performed_at`), уже лежат в
ней; колонки, которые хранят настоящие моменты (`workouts.started_at`,
`finished_at`, `sets.completed_at`, `profiles.updated_at`), остаются
`timestamptz` и в расчёт не подставляются.

#### Обязанности `Mapping/`, которые не выводятся из схемы

`Mapping/` (§20.2) — не только «схема §3.1 → типы FitCore»: в нескольких местах
он обязан посчитать, а не отобразить, и это часть контракта, потому что тот же
модуль будет линковать iOS-клиент.

- **Режим фаз для пользователя без `cycle_profiles`.** Строки может не быть
  вовсе (мужчина), либо `profiles.cycle_tracking = 'off'`. В обоих случаях
  `Mapping/` обязан подставить режим «без фаз» с причиной «выбор пользователя»,
  а не дефолт конструктора `CycleProfile`. Дефолт — фазовый режим, и в нём без
  опорной даты объём задаёт пустая периодизация: признак разгрузочной недели
  тогда игнорируется молча, у всех мужчин и у всех отключивших цикл разом
  (§11.5). Ни один тест §20.15 этого не ловит — ошибка видна только числом
  объёма.
- **`FatigueSet.muscleLoad`** — это уже произведение `contribution[m] ×
  fatigue_cost` (§8.1): перемножает их `Mapping/`, данные берутся из FitContent.
- **`PainEvent`** несёт множество суставов из `sets.pain_joints` (§8.4 п.5), а не
  вычисляет его заново при каждом чтении.
- **`EquipmentProfile`** подчиняется контракту §7.5 «флаг главнее своей весовой
  колонки»: при `has_kettlebells = false` набор гирь передаётся пустым, как бы
  он ни был заполнен.
- **Утомление в двух входах означает разное.** `WeekContext.fatigue` — состояние
  ДО распада (распад до 12:00 делает сам планировщик), `SessionInput.fatigue` —
  уже с распадом. Продеканное значение, положенное в `WeekContext`, применит
  распад дважды.
- **`WeekContext.exerciseHistory` — сырой журнал**, а не готовые состояния:
  готовое состояние могло быть посчитано с другой лестницей весов.
- **`machine_step_kg` подчиняется своему флагу так же, как гири.** Нет ни блока,
  ни тренажёров — шаг стека передаётся пустым, как бы ни была заполнена колонка.
  Правило то же и ошибка того же класса, что с `has_kettlebells`; названы они
  порознь только потому, что флагов два.
- **`sets.pain_joints` заполняется через `Recovery.painJoints(from:)`** в момент
  события, из `joint_stress` упражнения. Выше сказано, что читать их надо из
  колонки, а не выводить заново, — но кто и по какому правилу их туда кладёт,
  из этого не следует. Правило — «все суставы максимальной степени» (§8.4 п. 5),
  и второй реализации «максимума» заводить нельзя: разойдясь с ядром, она молча
  изменит счёт рекомендаций показаться специалисту.
- **`ExerciseSession.weightReadiness` берётся из
  `workout_exercises.weight_readiness`, а не из `workouts.readiness`.** Обе
  колонки существуют, обе `numeric`, обе называются «готовность» — это самая
  лёгкая молчаливая подмена во всём слое. Первая применена к весу ЭТОГО
  упражнения (§7.6), вторая — дневная (§10); демпфирование §9.6 читает только
  первую, и на второй базовая линия поедет.
- **Калибровочные подходы исключаются из утомления и из `неделя[m]`** (§9.8).
  Ни `Recovery.applying`, ни `Planner.weekDoneVolume` не фильтруют — они берут
  то, что им дали, а `CompletedWorkout.setsBySlug` описан просто как
  «выполненные подходы». Фильтр по `workout_exercises.is_calibration` — здесь.
  Колонка именно там, а не на тренировке: калибровка по §9.8 кончается по
  упражнению, и смешанная тренировка флагом на `workouts` не описывается ни
  одним его значением. `workouts.is_calibration` остаётся баннеру и аналитике и
  в расчёт не идёт вовсе. Проставляет колонку сервер при старте
  (`POST /v1/workouts`) из `exercise_states.in_calibration`, снимком, а не
  join'ом на чтении — по тому же доводу, по которому §19.2 п.8 зафиксировал
  `pain_joints` на момент события. Оттуда же берётся
  `ExerciseSession.isCalibration`: поле описывает одно упражнение одной
  тренировки, то есть ровно ту гранулярность, что и колонка.
- **`exercise_states.current_rep_min`/`current_rep_max` пишутся только вместе с
  сессией**, из `PrescribedExercise.targetRepMin`/`targetRepMax` — то есть из
  того самого вызова `Planner.repRange`, который дал
  `workout_exercises.target_rep_*` и диапазон дерева §20.9 (§9.1). Своего вызова
  `repRange` слой не делает: состояние после сбросов §9.7 накладывает сборка, и
  повторить его снаружи нечем. Для упражнений вне сессии колонки не трогаются —
  они снимок последней сессии, а не текущее значение (§3.1).
- **`SafetyProfile.restrictions` — только активные** (`resolved_at is null`,
  §3.1). Снятое ограничение, которое продолжает фильтровать, ничем себя не
  обнаруживает: пул упражнений просто уже, чем должен быть.
- **`SafetyProfile.isConservative` — это `parq_responses.has_red_flag`** (§14.1).
  Поле `acknowledged` в него не входит: оно говорит, прочитала ли
  пользовательница рекомендацию к врачу, а не сняла ли она флаг.

Отдельно — случай, который выглядит нарушением §20.7 и им не является:
**`CompletedWorkout.performedAt` строится из `workouts.finished_at`**, то есть из
колонки, отнесённой §20.7 к «настоящим моментам», которые в расчёт не
подставляются. Это допустимо, потому что значение используется ровно для одного —
упорядочить выполненные тренировки и найти последнюю (слагаемое w6, §7.3). Порядок
не меняется от общего сдвига шкалы, лишь бы все значения приходили из одной
колонки. Записано это здесь затем, чтобы никто не «починил» безвредное и не
подставил в него величину другой шкалы.
...
**Шкала FitCore — местная, и колонки разведены по ролям.** `Timestamp` и
`CalendarDay` отсчитываются от одной эпохи в местном времени: полночь дня `d` —
это `d × 24` часа, а момент оценки дня — `d × 24 + 12`. Поэтому колонки §3.1
делятся на две категории, и путать их нельзя:

| Роль | Колонки | Тип |
|---|---|---|
| Настоящий момент времени (аудит, синхронизация, LWW) | `workouts.started_at`, `workouts.finished_at`, `sets.completed_at`, `profiles.updated_at`, `cycle_profiles.updated_at` | `timestamptz` |
| Значение шкалы FitCore (участвует в расчёте) | `muscle_fatigue.updated_at` | `timestamp` без зоны |
| День шкалы FitCore | `exercise_states.last_performed_at` | `date` |

Единственное значение шкалы FitCore, которое строится из колонки первой
категории, — `CompletedWorkout.performedAt` (из `workouts.finished_at`). Это не
исключение из правила, а случай, где шкала не важна вовсе: значение служит
только для упорядочивания выполненных тренировок (§7.3, слагаемое w6), а порядок
не меняется от общего сдвига. Условие — все значения приходят из одной колонки;
подробности и остальные обязанности этого рода — в §20.6.

**Конверсия происходит один раз, при записи**, в зоне, присланной клиентом, и
больше не повторяется при чтении. Без этого распад §8.1 едет на смещение зоны:
утомление, посчитанное на 22:00 по московскому времени, прочитанное в
нью-йоркской зоне даёт до момента оценки не 14 часов, а 21 — 21% разницы по
мышце с полураспадом 20 ч, и это через порог §8.2. Для
`exercise_states.last_performed_at` цена та же в сутках: сессия в 01:00 местного
времени, прочитанная как момент в зоне западнее, переезжает на предыдущий день,
и §9.7 включает ступень `×0.92` на сутки раньше или позже.

**Переход на летнее время — не учитываемая погрешность.** Сдвиг на час внутри
одной зоны меняет разность меток на час, и этим пренебрегаем сознательно: на
полураспаде 20–40 ч это единицы процентов, а точный учёт потребовал бы хранить
зону или смещение у каждой строки — колонку, от которой §20.7 отказался.
...
**Форма дерева следует из `Progression.nextSet`.** Состояние узла — тройка
(вес по лестнице достижимых, был ли предыдущий фидбэк `failed`, сколько
повышений израсходовано в калибровке). Переходов из узла шесть, а не четыре,
потому что фактические повторы входят в решение предикатами, а не значением:
...
| `ok` | без изменения |
| `easy`, повторов не меньше `rep_max` | ×1.05; в калибровке ×1.15, пока не израсходованы три повышения (§9.8), дальше — без изменения |
| `easy`, повторов меньше `rep_max` | без изменения |

Две оговорки к таблице, без которых она расходится с кодом:

- **Вне калибровки повышение ограничено одной достижимой ступенью.** §9.3
  требует «никогда не повышаем более чем на один достижимый шаг за подход», и
  округление вверх само по себе этого не даёт: оно знает, куда положить
  результат, но не сколько ступеней при этом пройдено. На плотной лестнице ×1.05
  перелетает ступень, начиная с обычных рабочих весов. В калибровке ограничение
  не действует — её ×1.15 обязан прыгать через ступени, иначе поиск рабочего
  веса растягивается на сессии.
- **Направление округления зависит от ветки:** понижающие округляются вниз,
  повышающие — вверх.

На построение дерева обе не влияют (дерево строится вызовом самой `nextSet`),
но текст без них описывает не тот алгоритм, что работает.

**Третья компонента состояния — счётчик калибровочных повышений.** Он не
хранится ни на сервере, ни во фронтенде: лимит §9.8 действует в пределах одной
сессии одного упражнения, то есть ровно в пределах одного дерева, и выражается
его структурой. Само правило живёт в `Progression.nextSet`, которая принимает
счётчик параметром, — не в построении дерева: иначе веб и iOS считали бы его
по-разному, потому что там тот же экран вызывает `nextSet` напрямую.

**Упражнения без веса дерева с весами не получают.** При `ladder == .none`
(собственный вес, резинки) квантовать нечего, `baseline_kg` у такого упражнения
остаётся `nil` навсегда (§3.1), и стартового веса, от которого дерево строилось
бы, не существует вовсе. При этом одна ветка остаётся осмысленной и обязательной
— два `failed` подряд завершают упражнение досрочно (§9.3). Поэтому дерево
отдаётся, но без весов: только эта ветка. Рост повторами и подходами для таких
упражнений живёт между сессиями (§9.5) и внутрисессионному дереву не
принадлежит. Не отдавать дерево вовсе нельзя — фронтенду пришлось бы ходить на
сервер после каждого подхода отжиманий, то есть ровно за тем спиннером, ради
отсутствия которого §20.9 и существует.
...
дерево остаётся небольшим.

**Отсюда форма ответа: карта состояний, а не вложенное дерево.** Переход ведёт
не к вложенному потомку, а к идентификатору узла, и сошедшиеся ветки указывают
на один и тот же узел:

```
{"start": "n0",
 "nodes": {"n0": {"weight_kg": 10, "truncated": false, "terminates": false,
                  "transitions": {"easy_at_or_above_max": "n1", "ok": "n2", …}},
           …}}
```

Разница здесь не косметическая, и «дерево остаётся небольшим» верно только в
этой форме. Схождение — свойство состояний, а не путей: на четырёх переходах
различимых состояний 17–54 на реальных лестницах (гантели через 2 кг, стек
2.5 кг, штанга с блинами 1.25–10), тогда как вложенное кодирование
разворачивает их в 1483 узла — каждое состояние повторяется на каждом пути,
которым в него можно прийти. Платить пятьюдесятью кратами размера ответа
нечем: §20.9 существует ради латентности и ни ради чего больше.

**Предел — 512 узлов**, и это предохранитель, а не рабочий режим. Потолок
подходов §7.3 — пять, значит переходов не больше четырёх, а замер выше даёт на
этой глубине 17–54 узла: запас на порядок. Ни на одной проверенной лестнице
предел не срабатывает, в том числе в калибровке, где счётчик повышений
добавляет состояний. Если он всё же превышен, дерево
обрезается по глубине: узел, на котором обрезали, несёт признак `truncated`, и
фронтенд, дойдя до него, идёт на сервер. Признак стоит на узле, а не один на всё
дерево, потому что ветки сходятся неравномерно и обрезать их приходится на
разной глубине.
...
**`ETag` — SHA-256 от точных отдаваемых байт**, посчитанный один раз при старте
процесса; сильный, не `W/`; свой у каждого из двух путей. Из этого следуют ровно
те три свойства, которые нужны:

- правка контента меняет `ETag` обязательно — а §20.11 ниже прямо требует, чтобы
  опечатка в cue доезжала до клиента;
- передеплой без правок контента `ETag` НЕ меняет, и долгий кеш переживает
  выкатку сервера — иначе «долгий кеш» ничего не значит;
- правка формы DTO (`FitAPI`) меняет — и это верно, тело действительно другое.

Версия пакета для этого не годится: от правки опечатки она не меняется, пока её
не поднимут руками. Отметка сборки не годится тем более — она меняется на каждом
деплое и обесценивает кеш. Разные `ETag` у двух путей нужны, чтобы правка
шаблона растяжки не сбрасывала кеш библиотеки упражнений.
...
- 20b. Подделанная подпись, истёкший токен, токен другого проекта → 401, до БД
  запрос не доходит. Вторая половина — утверждение о том, что соединение из пула
  не арендовано ни разу, и наблюдается она на самой точке входа `DB/` (§20.4), а
  не на параллельной заглушке: заглушка, которой продакшен-путь не пользуется,
  не доказывает ничего.
- 20k. Изоляция на уровне `DB/`, без HTTP: два настоящих JWT двух пользователей
  через точку входа в транзакцию → каждый видит свои строки и ноль чужих.
  Отдельно — что `fitserver` без подстановки не читает ничего
  (`permission denied`), а не читает всё. Предшественник 20a и **не** его
  замена: 20a обязателен отдельно, когда появятся эндпоинты (§20.4).
- 20c. Каждая ветка отданного дерева §20.9 совпадает с прямым вызовом
  ...
  **Ещё две половины, слепые по той же причине.** Лимит калибровки: четвёртое
  «легко» подряд в калибровке вес не поднимает (§9.8). Сверка с `nextSet` его
  поймает только теперь, когда счётчик стал её параметром, — но утверждение
  ставится явно, потому что до этого именно здесь расхождение и было невидимым.
  Упражнение без веса: у дерева при `ladder == .none` весов нет вовсе, есть
  только ветка досрочного завершения (§20.9); отданные числа прошли бы сверку с
  `nextSet` зелёными, потому что врали бы одинаково с обеих сторон.

- 20j. Повторный `POST /v1/week-plans` на существующую неделю не создаёт второй
  и не перегенерирует первую: ответ равен первому, `week_plans.generated_at` и
  состав дней не изменились (§20.3). Отдельно — что запрос на прошлую неделю
  отклоняется.
...
- 20g. Логирование подхода с веба двигает `sync_seq` родительской `workouts`
  (§3.1); то же для `POST /v1/workouts/{id}/finish` и для замены упражнения.
  Отдельно — идемпотентный ретрай (тот же клиентский `id`, те же значения):
  курсор **не** двигается, потому что триггер обновления снабжён условием
  `old.* is distinct from new.*`; настоящее изменение строки — двигает.
  Формулировка 37c («изменён подход уже выгруженной тренировки») по HTTP
  неприменима и здесь не повторяется: подход приезжает уже завершённым, а 20f
  запрещает менять завершённый — путь «изменение» существует только на стороне
  `FitData`, где иммутабельность проверяет другой слой.
...
- 20i. Утомление, записанное в одной зоне и прочитанное в другой, даёт тот же
  распад: `muscle_fatigue.updated_at` и `exercise_states.last_performed_at`
  лежат на шкале FitCore (§20.7), и смена присланной зоны между запросами не
  меняет ни величину распада §8.1, ни ступень детренированности §9.7. Тест
  нужен отдельно, потому что ошибка здесь не падает и не видна в ответе — она
  проявляется только весом, посчитанным от неверного утомления.
