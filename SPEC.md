...
по четыре секунды или тринадцать по три, середина того, что дают диапазоны §6.2.
Формула от повторов была бы честнее, но читала бы величину, которой ещё нет:
`target_rep_min`/`target_rep_max` назначаются уже выбранному упражнению, и откуда
они берутся — не определено (§9.5, «Не реализовано (1 из 3)»). Она тянула бы и
`rep_extension` (§9.5): пользователь на `rep_max + 4` делает заметно более долгий
подход. **Когда §9.5 закроется, сюда вернуться:** работа станет функцией
назначенного диапазона, и `default_rep_range` войдёт в срез §7.5 вместе с ней.
...
- `default_rep_range` — диапазон назначается уже выбранному упражнению; откуда
  ...
  реализовано (1 из 3)»). Формула времени §7.3 берёт работу в подходе константой
  именно поэтому; когда §9.5 закроется, поле войдёт в срез вместе с ней.
