...
create table week_plans (
  ...
  -- дней нет строки workouts, а workout_exercises висит на ней. Снимок пишется
  -- при каждой отдаче плана наружу и при пересборке; для отладки он не нужен,
  -- этим занят input_digest. null — неделя сгенерирована, но ни разу не
  -- показана: тогда пересборке не с чем сравнивать и строка не печатается.
