...
//
//  Обе колонки заполняет `Planner.repRange(goal:state:)` — та же функция, из
//  которой берутся `workout_exercises.target_rep_min`/`target_rep_max` и
//  диапазон дерева §20.9, и вызывается она на состоянии, которое использовала
//  сборка (после сбросов §9.7), а не на этой структуре до них. Иначе колонка
//  разойдётся с тем, что видела пользовательница, и ни один тест §20.15 этого
//  не покажет.
