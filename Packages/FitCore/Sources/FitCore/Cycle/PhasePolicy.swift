...
extension Cycle {
    ...
    /// функцию подбора (Planner, кода ещё нет) для упражнений с
    ...
    /// правило).
    ///
    /// Величина 0.3 — шкала штрафа, а не готовый вклад в score: Planner
    /// умножает её на вес w7 = 3.0, так что максимум штрафа — 0.9 эффективного
    /// подхода при полной уверенности (SPEC §7.3, таблица весов). Когда этот
    /// doc-комментарий писался, чисел у целевой функции не было вовсе и 0.3
    /// была задокументированным выбором; теперь и шкала, и вес зафиксированы в
    /// SPEC. Штраф остаётся мягким: упражнение уходит, когда есть замена того
    /// же `pattern`, и остаётся, когда её нет — фаза никогда не фильтр.
