...
public enum Recovery {
    ...
    ///
    /// SPEC §8.1 покрывает все 19 слагов §6.4 явно. Восемь мышц (`gluteMed`,
    /// `trapsMid`, `trapsUpper`, `rearDelts`, `frontDelts`, `forearms`, `abs`,
    /// `obliques`) получают консервативный дефолт «мелких» (20ч): SPEC §8.2
    /// требует не блокировать тренировку утомлением, а более быстрый распад
    /// для мышцы без отдельного клинического обоснования реже завышает её
    /// утомление там, где это не подтверждено. `adductors` — не в этой
    /// группе: приводящие бедра работают как разгибатель бедра наравне с
    /// ягодичными и бицепсом бедра, поэтому отнесены к крупным (30ч), а не к
    /// дефолту.
    ///
    ...
    /// пока ей не назначили период полураспада явно. Восемь мышц с дефолтом
    /// вынесены отдельной веткой, а не слиты с «мелкими» по значению, чтобы
    /// граница между тем, что в SPEC обосновано клинически, и тем, что —
    /// консервативным выбором, оставалась видна в коде.
    public static func fatigueHalfLifeHours(for muscle: MuscleSlug) -> Double {
        switch muscle {
        case .gluteMax, .quads, .hamstrings, .lats, .pecs, .adductors:
            return 30
        case .biceps, .triceps, .sideDelts, .calves:
            return 20
        case .erectors:
            return 40
        case .gluteMed, .trapsMid, .trapsUpper, .rearDelts,
