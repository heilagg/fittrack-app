...
extension Readiness {
    ...
    /// Пороги готовности §10 — все, какие есть. SPEC вводить новые запрещает
    /// («новых порогов не вводим», §7.3), и читать их обязаны все: ±1 подход на
    /// сессию, надбавка к RIR, множитель отдыха в бюджете времени и таймере
    /// (§7.3, §13.3) и слагаемое w9 целевой функции. Одно число в одном месте:
    /// свой литерал у потребителя разводит полосы молча.
    public enum Thresholds {
        /// Ниже — −1 подход на сессию и отдых ×1.2. Граница строгая.
        public static let setDecrease = 0.9
        /// Выше — +1 подход на сессию и отдых ×0.8. Граница строгая.
        public static let setIncrease = 1.05
        /// Ниже — целевой RIR +1 (§10) и штраф w9 за технически сложную базу (§7.3).
        public static let lowReadiness = 0.85
    }
