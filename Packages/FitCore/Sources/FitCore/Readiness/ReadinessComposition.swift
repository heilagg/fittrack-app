...
extension Readiness {
    ...
    /// порядку сессии — `indices.min(by:)` при равенстве оставляет первый
    /// встреченный минимум (проверено эмпирически: `min(by:)` и `max(by:)` в
    /// Swift оба оставляют первый элемент при ничьей, здесь асимметрии нет —
    /// см. тот же вывод в `Recovery.primaryJoint(from:)`, где `max(by:)`
    /// используется намеренно ради этого же свойства). Такое же зеркальное
    /// правило, что у выбора для `+1` выше (первый подходящий из
    /// равнозначных): упражнения с одинаковым `worstVolumeMultiplier`
    /// эквивалентны по этому правилу, и порядок брать неважно, но
    /// детерминированный выбор нужен для воспроизводимости (задокументированный
    /// выбор, не факт из SPEC — SPEC не разбирает случай нескольких
    /// равно-утомлённых упражнений).
