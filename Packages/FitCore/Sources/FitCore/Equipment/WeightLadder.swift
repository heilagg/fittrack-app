...
public enum WeightLadder: Sendable, Equatable {
    ...
    /// `build()`: там пять мест, конструирующих `.discrete`, три — элементы
    /// после `round2`, два — пустой литерал `[]` (нет штанги / нет шага
    /// тренажёра), и оба варианта кратны центу (пустой список — тривиально).
    /// Конструирование вручную с нецентовым весом — обязанность вызывающего
    /// кода нарушена: `nextAchievableWeight` полагается на кратность центу
    /// при сравнении и падает через `precondition`, если это не так (см.
    /// `isCentMultiple`). Код-ревью feature/weight-ladder, 2026-09-04.
